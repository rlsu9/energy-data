...
# Bypass the US proxy and access directly
US_PROXY = 'https://aeepr.com'
HOST_PARAMETER = ''
...
def extract_data(html):
  ...
  dataSource = re.search(r"dataSource: (\{.+\}\]\})\}\);", html).group(1)#Extract object with data
  dataSource = re.sub(r",\s*\}", "}", dataSource) #,} is valid JavaScript but not valid JSON
...
def fetch_production(zone_key='US-PR', session=None, target_datetime=None, logger: logging.Logger = logging.getLogger(__name__)) -> dict:
...
#    },
  ...
  if(renewable_output >= 0.0):
    ...
    logger.warning(f"Renewable generation breakdown page total is greater than total renewable output, a difference of {renewable_output}MW", extra={"key": zone_key})

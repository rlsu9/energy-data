...
# Note: HI data either stoppped after 4/14/2022, or it's lagging by two weeks
# Source: https://github.com/electricitymap/electricitymap-contrib/blob/master/parsers/US_HI.py
...
EARLIEST_PROD_DT = arrow.get('2013/12/04 08:30', tzinfo='Pacific/Honolulu')  # this is the earliest datetime with recorded data
...
def get_historical_prod(r, request_dt):
    ...
    if request_dt.hour == 23 and (31 <= request_dt.minute <= 59):
        query_url = BASE_URL + 'date={}'.format(request_dt.shift(days=+1).format('YYYY-MM-DD'))
    else:
    ...
    res[0]['dateTime'] = arrow.get(res[0]['dateTime']).to(tz="Pacific/Honolulu").datetime
